...
into the ``laundry_assumption_overrides`` JSONB column on a settings table
(see :mod:`app.laundry.services.settings_service`). The numbers below are the
*default* book values used by the deterministic financial model.
...
class MachineAssumptions:
    ...
    # clearance, drain pan, service access, and aisle re-use.
    ...
    # We keep the model conservative so manual-review remains the default landing zone.
...
class ScoringWeights:
    ...
    location: float = 0.32
    economics: float = 0.30
    physical_fit: float = 0.16
...
class ScoringThresholds:
    """Score → lifecycle bucket. Aligned with the brief's 75/50 split."""

    approved_min: int = 75
    manual_review_min: int = 50
...
class LaundryAssumptions:
    ...
    rent_capex_discount_pct: float = 0.0       # rent deals don't carry purchase price
