...
from jobs.db_health import assert_pipeline_ready, check_missing_tables, _is_missing_table_error
from jobs.errors import DatabaseSetupError, StoreError
from jobs.logging_util import safe_json

T = TypeVar("T")

# Job-level pipeline steps use listing_index = -1 (NULL breaks UNIQUE in PostgreSQL)
JOB_LEVEL_INDEX = -1
...
def _raise_from_api_error(exc: APIError, *, table: str, operation: str) -> None:
    if _is_missing_table_error(exc):
        ...
        if not missing:
            table_name = table
            missing = [table_name]
        raise DatabaseSetupError(missing) from exc
...
def _execute(fn: Callable[[], T], *, table: str, operation: str) -> T:
    try:
        ...
        if _is_missing_table_error(exc):
            missing = check_missing_tables(force=True)
            raise DatabaseSetupError(missing or [table]) from exc
        raise StoreError(
...
def start_step(job_id: str, step_key: str, listing_index: int = JOB_LEVEL_INDEX) -> dict:
    ...
    if not res.data:
        raise KeyError(f"Step not found: {step_key} (listing_index={listing_index})")
...
def build_job_response(job_id: str) -> dict:
    ...
    manual = [
        r for r in results
        if isinstance(r, dict) and r.get("deal_status") == "manual_review"
    ]
    rejected = [
        r for r in listings
