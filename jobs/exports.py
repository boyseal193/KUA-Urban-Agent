...
def flatten_result(listing_row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "verdict_detail": score.get("verdict_detail"),
        "classification": score.get("classification"),
        "deal_killer": score.get("deal_killer"),
        "scoring_version": score.get("scoring_version") or economics.get("scoring_version"),
        "confidence_pct": (score.get("confidence") or {}).get("pct") if isinstance(score.get("confidence"), dict) else None,
        "failed_gates": " | ".join(score.get("gate_failures") or []) if isinstance(score.get("gate_failures"), list) else "",
        # --- Address / location ---
        ...
        "transaction_costs": _money(economics.get("acquisition_transaction_cost_eur")),
        "price_per_m2": _money(economics.get("price_per_m2_eur")),
        "downside_ebitda": _money(economics.get("downside_ebitda_eur")),
        "severe_downside_ebitda": _money(economics.get("severe_downside_ebitda_eur")),
        "downside_yield": _pct(economics.get("downside_yield_pct")),
        "rent_to_revenue": _pct(economics.get("rent_to_revenue_pct")),
        # --- Sub-scores ---
